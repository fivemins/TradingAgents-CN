...
function CandidateSection({ title, items, emptyText, showRejectedReason, showActions, busyKey, scanId, onAnalyze, onOpenTask }: { title: string; items: OvernightCandidate[]; emptyText: string; showRejectedReason?: boolean; showActions?: boolean; busyKey: string; scanId?: string; onAnalyze: (candidate: OvernightCandidate) => void; onOpenTask: (taskId: string) => void; }) {
  return (
      ...
      {items.length ? (
        <div className="breakdown-table">
          <table>
            <thead>
              <tr>
                <th>股票代码</th><th>名称</th><th>板块</th><th>Quick Score</th><th>Total Score</th><th>尾盘质量</th><th>{showRejectedReason ? "淘汰原因" : "当前去向"}</th><th>因子拆解</th><th>任务</th>
              </tr>
            </thead>
            <tbody>
              {items.map((candidate) => (
                <tr key={`${title}:${candidate.ticker}`}>
                  ...
                  <td>{showActions ? (candidate.linked_task_id ? <button type="button" className="table-link-button" onClick={() => onOpenTask(candidate.linked_task_id!)}>查看分析</button> : <button type="button" className="table-link-button" disabled={busyKey === `${scanId}:${candidate.ticker}`} onClick={() => onAnalyze(candidate)}>{busyKey === `${scanId}:${candidate.ticker}` ? "创建中..." : "深度分析"}</button>) : <span>{candidate.linked_task_id ? "已创建" : "--"}</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export function OvernightPage() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        if (!cancelled) {
          setState((current) => ({ ...current, scanList, reviewList, selectedScan, selectedReview, selectedReviewArtifacts }));
        }
      } catch (err) {
  ...
  }, [setError, setState, shouldPollReview, shouldPollScan, state.selectedReview?.review_id, state.selectedScan?.scan_id]);
  ...
  const createReview = async () => {
    ...
    try {
      ...
      const [reviewList, selectedReviewArtifacts] = await Promise.all([dashboardApi.getOvernightReviews(), dashboardApi.getOvernightReviewArtifacts(detail.review_id)]);
    ...
    <div className="content-shell">
      <section className="hero-card">
        <div>
          ...
          <p className="hero-copy">这里集中展示正式推荐、观察名单、淘汰原因与历史验证结果，运行中的任务会直接显示“运行中”和当前步骤。</p>
        </div>
        <div className="hero-actions">
          ...
          <button type="button" className="secondary-button" disabled={creatingReview} onClick={() => void createReview()}>{creatingReview ? "正在创建验证..." : "发起历史验证"}</button>
        </div>
      </section>
        ...
        <div className="analysis-main-grid">
          <div className="analysis-block">
            <div className="analysis-form-grid">
              ...
              <label className="analysis-field"><span>扫描模式</span><select value={mode} onChange={(event) => setMode(event.target.value as OvernightMode)}><option value="strict">严格模式</option><option value="research_fallback">研究回退</option></select></label>
            </div>
            <div className="hero-actions">
      ...
      <section className="panel panel-wide">
        ...
        <div className="scan-history-row">
          {loading ? <div className="empty-state">正在加载扫描记录...</div> : state.scanList?.items.length ? state.scanList.items.slice(0, 8).map((item) => {
            ...
            return <button key={item.scan_id} type="button" className={scan?.scan_id === item.scan_id ? "scan-chip scan-chip-active" : "scan-chip"} onClick={() => void dashboardApi.getOvernightScan(item.scan_id).then((selectedScan) => setState((current) => ({ ...current, selectedScan })))}><strong>{item.trade_date}</strong><div className="scan-chip-status"><StatusBadge value={item.status} /><span className="scan-chip-step">{itemProgress.currentStepLabel}</span></div><span className="scan-chip-progress">{item.status === "succeeded" ? `${item.formal_count} 只正式推荐` : itemProgress.detailLabel}</span></button>;
        ...
        {scan ? <div className="overnight-home-grid">
          ...
          <article className="overnight-highlight-card"><div className="decision-head"><strong>本次扫描摘要</strong></div><div className="analysis-estimate-list"><div className="analysis-estimate-row"><span>扫描日期</span><strong>{scan.trade_date}</strong></div><div className="analysis-estimate-row"><span>模式</span><strong>{fmtMode(scan.mode)}</strong></div><div className="analysis-estimate-row"><span>正式推荐</span><strong>{scan.formal_count}</strong></div><div className="analysis-estimate-row"><span>Total Score 候选</span><strong>{scan.scored_count}</strong></div><div className="analysis-estimate-row"><span>已淘汰</span><strong>{scan.rejected_count}</strong></div><div className="analysis-estimate-row"><span>配置哈希</span><strong className="notranslate" translate="no" lang="en">{scan.evaluation_config_hash ?? "--"}</strong></div></div></article>
          <article className="overnight-highlight-card"><div className="decision-head"><strong>数据质量与来源</strong></div><div className="analysis-estimate-list"><div className="analysis-estimate-row"><span>市场提示</span><strong className="text-multiline">{scan.market_message}</strong></div><div className="analysis-estimate-row"><span>数据质量</span><strong>{String(scan.data_quality?.status ?? "--")}</strong></div><div className="analysis-estimate-row"><span>快照日期</span><strong>{scan.universe_snapshot_date ?? "--"}</strong></div><div className="analysis-estimate-row"><span>Provider Route</span><strong className="text-multiline">{routeText(scan.provider_route)}</strong></div></div><p className="card-note">{scan.bias_flags.length ? `偏差标记：${scan.bias_flags.join(" / ")}` : "当前没有额外偏差标记。"}</p></article>
      ...
      <CandidateSection title="正式推荐" items={scan?.formal_recommendations ?? []} emptyText="当前没有正式推荐。" showActions busyKey={submittingTaskKey} scanId={scan?.scan_id} onAnalyze={(candidate) => void runDeepAnalysis(candidate)} onOpenTask={(taskId) => navigate(`/tasks/${taskId}`)} />
      <CandidateSection title="Total Score 候选池" items={scan?.total_score_candidates ?? []} emptyText="当前没有完成总评分的候选。" busyKey={submittingTaskKey} scanId={scan?.scan_id} onAnalyze={(candidate) => void runDeepAnalysis(candidate)} onOpenTask={(taskId) => navigate(`/tasks/${taskId}`)} />
      <CandidateSection title="淘汰原因" items={scan?.rejected_candidates ?? []} emptyText="当前没有已淘汰候选。" showRejectedReason busyKey={submittingTaskKey} scanId={scan?.scan_id} onAnalyze={(candidate) => void runDeepAnalysis(candidate)} onOpenTask={(taskId) => navigate(`/tasks/${taskId}`)} />
      <CandidateSection title="通过初筛的股票" items={scan?.preliminary_candidates ?? []} emptyText="当前没有通过初筛的股票。" busyKey={submittingTaskKey} scanId={scan?.scan_id} onAnalyze={(candidate) => void runDeepAnalysis(candidate)} onOpenTask={(taskId) => navigate(`/tasks/${taskId}`)} />
      <CandidateSection title="观察名单" items={scan?.watchlist ?? []} emptyText="当前没有观察名单。" showActions busyKey={submittingTaskKey} scanId={scan?.scan_id} onAnalyze={(candidate) => void runDeepAnalysis(candidate)} onOpenTask={(taskId) => navigate(`/tasks/${taskId}`)} />

      <section className="panel panel-wide">
        <div className="panel-header"><div><p className="eyebrow">历史验证</p><h3>近 60 个交易日回看</h3></div></div>
        <div className="scan-history-row">
          {loading ? <div className="empty-state">正在加载历史验证记录...</div> : state.reviewList?.items.length ? state.reviewList.items.slice(0, 6).map((item) => {
            const itemProgress = getOvernightExecutionState("review", item.status, item.progress_message);
            return <button key={item.review_id} type="button" className={review?.review_id === item.review_id ? "scan-chip scan-chip-active" : "scan-chip"} onClick={() => void Promise.all([dashboardApi.getOvernightReview(item.review_id), dashboardApi.getOvernightReviewArtifacts(item.review_id)]).then(([selectedReview, selectedReviewArtifacts]) => setState((current) => ({ ...current, selectedReview, selectedReviewArtifacts })))}><strong>{item.end_trade_date}</strong><div className="scan-chip-status"><StatusBadge value={item.status} /><span className="scan-chip-step">{itemProgress.currentStepLabel}</span></div><span className="scan-chip-progress">{item.status === "succeeded" ? `${item.summary_snapshot?.days_with_formal_picks ?? 0} 个有推荐交易日` : itemProgress.detailLabel}</span></button>;
          }) : <div className="empty-state">还没有历史验证记录。</div>}
        </div>

        {review ? <div className="overnight-review-layout">
          <article className="overnight-highlight-card"><OvernightExecutionStatus title="历史验证执行状态" kind="review" status={review.status} progressMessage={review.progress_message} startedAt={review.started_at} finishedAt={review.finished_at} errorMessage={review.error_message} /></article>
          {reviewSummary ? <><div className="overnight-home-grid"><article className="overnight-highlight-card"><div className="decision-head"><strong>验证摘要</strong></div><div className="analysis-estimate-list"><div className="analysis-estimate-row"><span>截止日期</span><strong>{review.end_trade_date}</strong></div><div className="analysis-estimate-row"><span>窗口</span><strong>{review.window_days} 交易日</strong></div><div className="analysis-estimate-row"><span>有推荐交易日</span><strong>{reviewSummary.days_with_formal_picks}</strong></div><div className="analysis-estimate-row"><span>平均次日开盘收益</span><strong>{fmtPct(reviewSummary.avg_next_open_return)}</strong></div><div className="analysis-estimate-row"><span>平均超额收益</span><strong>{fmtPct(reviewSummary.avg_excess_return)}</strong></div><div className="analysis-estimate-row"><span>正收益命中率</span><strong>{typeof reviewSummary.positive_pick_rate === "number" ? `${(reviewSummary.positive_pick_rate * 100).toFixed(1)}%` : "--"}</strong></div></div></article><article className="overnight-highlight-card"><div className="decision-head"><strong>可复现性说明</strong></div><div className="analysis-estimate-list"><div className="analysis-estimate-row"><span>配置版本</span><strong>{review.evaluation_config_version ?? "--"}</strong></div><div className="analysis-estimate-row"><span>配置哈希</span><strong className="notranslate" translate="no" lang="en">{review.evaluation_config_hash ?? "--"}</strong></div><div className="analysis-estimate-row"><span>快照日期</span><strong>{review.universe_snapshot_date ?? "--"}</strong></div><div className="analysis-estimate-row"><span>存活偏差</span><strong>{review.survivorship_bias ? "是" : "否"}</strong></div></div><p className="card-note">{routeText(review.provider_route)}</p></article></div><div className="structured-summary-grid"><BreakdownTable title="按市场 Regime" items={review.regime_breakdown ?? []} emptyText="当前没有市场 Regime 分层结果。" /><BreakdownTable title="按板块池" items={review.pool_breakdown ?? []} emptyText="当前没有板块池分层结果。" /><BreakdownTable title="按尾盘质量" items={review.tail_quality_breakdown ?? []} emptyText="当前没有尾盘质量分层结果。" /></div><article className="analysis-note-card"><strong>最近几日表现</strong>{recentDailyResults.length ? <div className="breakdown-table"><table><thead><tr><th>交易日</th><th>正式推荐</th><th>候选代码</th><th>等权次日开盘收益</th><th>沪深 300 次日开盘收益</th><th>平均超额收益</th></tr></thead><tbody>{recentDailyResults.map((item) => <tr key={item.trade_date}><td>{item.trade_date}</td><td>{item.formal_count}</td><td className="notranslate" translate="no" lang="en">{item.formal_tickers?.length ? item.formal_tickers.join(", ") : "--"}</td><td>{fmtPct(item.equal_weight_next_open_return)}</td><td>{fmtPct(item.benchmark_next_open_return)}</td><td>{fmtPct(item.avg_excess_return)}</td></tr>)}</tbody></table></div> : <div className="empty-state compact-empty">当前没有可展示的最近几日表现。</div>}</article></> : <div className="empty-state">当前历史验证尚未生成可展示的统计摘要。</div>}
        </div> : <div className="empty-state">请先发起一次历史验证。</div>}
      </section>
    </div>
  );
}
